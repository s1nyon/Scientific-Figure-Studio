"""Render an explicit illustrative model architecture."""
...
from figure_studio.illustrations import draw_architecture  # noqa: E402
...
def load_data(path: str | Path) -> dict[str, object]:
    """Read modules and connections without inferring missing model structure."""
    ...
    if not isinstance(data, dict) or not isinstance(data.get("modules"), dict):
        raise ValueError("architecture JSON requires a modules mapping")
    if not isinstance(data.get("connections"), list):
        raise ValueError("architecture JSON requires a connections list")
    return data


def build_figure(data: dict[str, object], config: dict[str, object]) -> plt.Figure:
    """Draw only the supplied modules and connections."""
    ...
    with figure_style(str(config["style_name"]), canvas="wide"):
        with plt.rc_context({"font.size": float(config["font_size"])}):
            ...
            draw_architecture(axis, data["modules"], data["connections"], config)
            axis.set_title("Illustrative model architecture", loc="left", pad=12)
            evidence = str(
                data.get(
                    "evidence_note",
                    "Illustrative example structure; not a user-provided model.",
                )
            )
...
def main(
    ...
    """Generate architecture artifacts and return their paths."""
                ...
                provenance=build_data_provenance(
                    manifest,
                    {
                        ...
                        "architecture_semantics": (
                            "Connections are source-to-target edges from input JSON."
                        ),
                    },
                ),
            )
