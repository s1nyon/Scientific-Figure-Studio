"""Render the round-02 Nature-first structure redesign.

The figure keeps the repository practice data unchanged and reorganizes the
evidence into one hero trajectory panel plus one subordinate signed-update
panel.  It intentionally does not infer speed, stability, or uncertainty from
the single deterministic trajectory supplied for each algorithm.
"""
...
def _add_endpoint_labels(
    ...
    """Place short direct labels at the terminal points without a legend box."""
...
def build_figure(
    ...
    """Build the hero trajectory and signed-update evidence figure."""
    ...
    with figure_style(str(config["style_name"]), canvas="composite"):
        with plt.rc_context(
            ...
            delta_axis = figure.add_subplot(grid[1], sharex=hero)
            ...
            # Hero: raw current objective is the primary evidence. The actual
            # values are not smoothed or altered for visual presentation.
            for algorithm in algorithms:
                values = series[algorithm]
                hero.plot(
                    values["iteration"],
            ...
            hero.tick_params(axis="x", labelbottom=False)
            ...
            # Running-best values coincide with current values everywhere except
            # Proposed at iteration 6. Show only that informative local gap.
            ...
            if proposed is not None:
                ...
                if len(mismatch) == 1:
                    ...
                    current_value = float(proposed["current"][index])
                    ...
                    hero.annotate(
                        "current > incumbent",
                        xy=(x_value, current_value),
                        xytext=(x_value + 0.35, current_value + 0.16),
            ...
            # Supporting panel: signed adjacent-step changes on the same x scale.
            ...
            delta_axis.set_xticks(np.arange(1, 11, dtype=float))
            delta_axis.set_xticklabels([str(int(value)) for value in np.arange(1, 11)])
            ...
            # The positive point is the only local regression in the supplied data.
            if proposed is not None:
                ...
                provenance=build_data_provenance(
                    manifest,
                    {
                        ...
                        "figure_design": {
                            "hero_panel": "a",
                            "support_panels": ["b"],
                            "current_vs_historical_best": "local divergence only",
                            "endpoint_summary_panel": False,
                            "delta_heatmap": False,
