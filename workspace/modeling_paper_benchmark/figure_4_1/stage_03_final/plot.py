...
except ModuleNotFoundError:  # pragma: no cover - direct execution from stage folder
    import sys
...
def _load_structure(data_path: Path | None) -> dict[str, Any]:
    path = data_path or config.ROOT / "structure_final.json"
...
def _boundary_point(node: dict[str, Any], side: str) -> tuple[float, float]:
    x = float(node["x"])
    y = float(node["y"])
    width = float(node["width"])
    height = float(node["height"])
...
def _route_points(
    source: dict[str, Any], target: dict[str, Any], route: str
) -> list[tuple[float, float]]:
    start_side = "right" if route == "horizontal" else "bottom"
    start = _boundary_point(source, start_side)
    if route == "horizontal":
        return [start, _boundary_point(target, "left")]
    end = _boundary_point(target, "top")
    if route == "down":
        return [start, end]
    gap_mid_y = (start[1] + end[1]) / 2
    if route in {"down_left", "down_right"}:
        return [start, (end[0], gap_mid_y), end]
    if route in {"merge_left", "merge_right"}:
        offset = -0.055 if route == "merge_left" else 0.055
        merge_x = float(target["x"]) + offset
        return [start, (merge_x, gap_mid_y), (merge_x, end[1])]
    raise ValueError(f"unknown edge route: {route}")


def _draw_polyline_edge(ax: Any, points: list[tuple[float, float]]) -> None:
    if len(points) < 2:
        raise ValueError("an edge route requires at least two points")
    ax.plot(
...
def _draw_edges(ax: Any, structure: dict[str, Any], nodes: dict[str, dict[str, Any]]) -> None:
    for edge in structure.get("edges", []):
        ...
        points = _route_points(source, target, str(edge.get("route", "horizontal")))
        _draw_polyline_edge(ax, points)
        if edge.get("label"):
            mx = (points[0][0] + points[-1][0]) / 2
            my = (points[0][1] + points[-1][1]) / 2 + 0.026
            ax.text(
                mx,
                my,
                str(edge["label"]),
