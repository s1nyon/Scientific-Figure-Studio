...
from figure_studio.artifacts import promote_candidate
...
def main(output_dir=None, data_path=None, manifest_path=None, skill_root=None, overwrite=False):
    ...
    if data_path is not None:
        assert Path(data_path).parent == destination
...
def _write_failing_renderer(folder: Path) -> Path:
    renderer = folder / "failing_plot.py"
    renderer.write_text(
        "def main(output_dir=None, **kwargs):\n"
        "    raise RuntimeError('renderer failure for safety test')\n",
        encoding="utf-8",
    )
    return renderer
...
def test_run_figure_task_writes_outputs_receipts_and_source_hashes(tmp_path):
    ...
    reproduction = json.loads(
        (output_dir / "reproduction_manifest.json").read_text(encoding="utf-8")
    )
    assert reproduction["dependencies"] == ["matplotlib", "numpy"]
    assert reproduction["project_independent"] is False
...
def test_run_figure_task_checks_output_conflict_before_copying_delivery_sources(tmp_path):
    source_folder = tmp_path / "source"
    source_folder.mkdir()
    renderer = _write_renderer(source_folder)
    data_path = source_folder / "practice.csv"
    ...
    output_dir = tmp_path / "output"
    output_dir.mkdir()
    existing = output_dir / "figure.png"
    existing.write_bytes(b"existing accepted-looking bytes")
    ...
    assert existing.read_bytes() == b"existing accepted-looking bytes"
    assert not (output_dir / "plot.py").exists()


def test_renderer_failure_does_not_leave_partial_delivery_files(tmp_path):
    source_folder = tmp_path / "source"
    source_folder.mkdir()
    renderer = _write_failing_renderer(source_folder)
    data_path = source_folder / "practice.csv"
    data_path.write_text("x,y\n0,2\n1,1\n2,0.8\n", encoding="utf-8")
    manifest = _write_manifest(source_folder, data_path)
    brief = source_folder / "figure_brief.json"
...
def test_runner_refuses_to_write_an_accepted_version_even_with_overwrite(tmp_path):
    source_folder = tmp_path / "source"
    source_folder.mkdir()
    renderer = _write_renderer(source_folder)
    data_path = source_folder / "practice.csv"
    ...
    accepted = promote_candidate(
        candidate,
        tmp_path / "accepted",
        user_note="explicit test acceptance",
    )
    original_hash = hashlib.sha256((accepted / "figure.png").read_bytes()).hexdigest()
    ...
    assert hashlib.sha256((accepted / "figure.png").read_bytes()).hexdigest() == original_hash
